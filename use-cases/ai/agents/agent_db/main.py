...
def setup_db_tunnel():
    ...
    skip = os.environ.get("AGENT_DB_SKIP_DISCOVERY", "").lower() in ("1", "true", "yes")
    if skip:
        logger.info("discovery disabled via AGENT_DB_SKIP_DISCOVERY, using DATABASE_URL from environment")
        return
